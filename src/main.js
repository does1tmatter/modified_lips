...
const cleanDna = (_str) => {
  const withoutOptions = _str.includes('?') ? removeQueryStrings(_str) : _str;
  var dna = withoutOptions.includes(':') ? withoutOptions.split(":")[1] : withoutOptions;
  return dna;
};

const cleanName = (_str) => {
  let nameWithoutExtension = _str.endsWith('.png') ? _str.slice(0, -4) : _str;
  var nameWithoutWeight = nameWithoutExtension.includes(rarityDelimiter) ? nameWithoutExtension.split(rarityDelimiter).shift() : nameWithoutExtension;
  return nameWithoutWeight;
};
...
const constructLayerToDna = (_dna = "", _layers = []) => {
  let mappedDnaToLayers = _layers.map((layer, index) => {
    let selectedElement = layer.elements.find(
      (e) => e.name == cleanDna(_dna.split(DNA_DELIMITER)[index])
    );
    return {
      name: layer.name,
      blend: layer.blend,
...
const createDna = (_layers) => {
  ...
  _layers.forEach((layer) => {
    ...
    for (var i = 0; i < layer.elements.length; i++) {
      ...
      if (random < 0) {
        return randNum.push(
          `${layer.elements[i].filename.split(rarityDelimiter)[0]}${
...
const saveMetaDataSingleFile = (_editionCount) => {
  let metadata = metadataList.find((meta) => meta.image == `${baseUri}/${_editionCount}.png`);
